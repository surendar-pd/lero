...
#         print(f"Model weights saved to {_nn_path(path)}")

#         with open(_feature_generator_path(path), "wb") as f:
#             joblib.dump(self._feature_generator, f)
#             print(f"Feature generator saved to {_feature_generator_path(path)}")

#         with open(_input_feature_dim_path(path), "wb") as f:
#             joblib.dump(self._input_feature_dim, f)
#             print(f"Input feature dimensions saved to {_input_feature_dim_path(path)}")
...
# Helper function to handle DataParallel and non-DataParallel cases
def get_model(model):
...
class LeroModel():
    ...
    def save(self, path):
        ...
        if CUDA:
            torch.save(get_model(self._net).state_dict(), _nn_path(path))
        else:
            torch.save(self._net.state_dict(), _nn_path(path))
        print(f"Model weights saved to {_nn_path(path)}")
    ...
    def fit(self, X, Y, pre_training=False):
        ...
        pairs = [(X[i], Y[i]) for i in range(len(Y))]
        dataset = DataLoader(pairs,
                             batch_size=batch_size,
                             shuffle=True,
                             collate_fn=collate_fn)
        ...
        if not pre_training:
            # Determine the initial number of input features
            input_feature_dim = len(X[0].get_feature())
            print("input_feature_dim:", input_feature_dim)
            ...
            self._input_feature_dim = input_feature_dim
            if CUDA:
                self._net = torch.nn.DataParallel(self._net, device_ids=GPU_LIST).cuda(device)

        optimizer = torch.optim.Adam(get_model(self._net).parameters())
        loss_fn = torch.nn.MSELoss()
        losses = []
        start_time = time()
        ...
        for epoch in range(100):
            ...
            for x, y in dataset:
                ...
                tree = get_model(self._net).build_trees(x)
...
class LeroModelPairWise(LeroModel):
    def fit(self, X1, X2, Y1, Y2, pre_training=False):
        ...
        pairs = [(X1[i], X2[i], 1.0 if Y1[i] >= Y2[i] else 0.0) for i in range(len(X1))]
        ...
        for epoch in range(100):
            loss_accum = 0
            for x1, x2, label in dataset:
                tree_x1 = get_model(self._net).build_trees(x1)
                tree_x2 = get_model(self._net).build_trees(x2)
